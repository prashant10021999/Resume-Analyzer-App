...
connection = pymysql.connect(host='localhost', user='root', password='Kumar@123#')
...
def insert_data(name, email, res_score, timestamp, no_of_pages, reco_field, cand_level, skills, recommended_skills, courses):
    ...
    # Truncate skills data if too long
    max_length = 255
    skills = skills[:max_length] if len(skills) > max_length else skills
    recommended_skills = recommended_skills[:max_length] if len(recommended_skills) > max_length else recommended_skills
    courses = courses[:max_length] if len(courses) > max_length else courses
